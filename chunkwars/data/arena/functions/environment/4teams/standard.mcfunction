#> arena:environment/4teams/standard
...
# we need to move until we hit the edge, thus 0, 32 respectectively, 40 down and then cover a volume of 32 x 32 x 255
execute as @e[tag=remove_water_red] at @s positioned ~-32 ~-40 ~ run execute if entity @a[team=!Spectator,dx=32,dy=255,dz=32] as @a[team=!Spectator,dx=32,dy=255,dz=32] at @s run fill ~-5 ~-5 ~-5 ~5 ~5 ~5 minecraft:air replace minecraft:water
...
# we need to move until we hit the edge, thus 0, 32 respectectively, 40 down and then cover a volume of 32 x 32 x 255
execute as @e[tag=remove_water_red] at @s positioned ~ ~-40 ~-32 run execute if entity @a[team=!Spectator,dx=32,dy=255,dz=32] as @a[team=!Spectator,dx=32,dy=255,dz=32] at @s run fill ~-5 ~-5 ~-5 ~5 ~5 ~5 minecraft:air replace minecraft:water
