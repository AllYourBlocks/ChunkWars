...
execute if score $teams gamestate matches 2 run tp @s ~ ~1 ~
...
execute unless score $teams gamestate matches 2 if block ~ ~ ~ minecraft:air run tp @s ~ ~-3 ~

# now, if our block is grass, we are just below the surface, let us move one more up to seal the deal
execute unless score $teams gamestate matches 2 if block ~ ~ ~ minecraft:grass_block run tp @s ~ ~1 ~
